...
export const updateCookProfileSchema = z.object({
  bio: z.string().trim().max(1000, 'Bio cannot exceed 1000 characters').optional(),
  experienceYears: z.number().min(0, 'Experience years cannot be negative').max(60, 'Experience years cannot exceed 60').optional(),
  serviceTypes: z
...
export const cookSearchQuerySchema = z.object({
  ...
  sort: z.enum(['averageRating', 'hourlyRate', 'experienceYears', 'createdAt']).default('averageRating'),
  order: z.enum(['asc', 'desc']).default('desc'),
});

export type UpdateCookProfileInput = z.infer<typeof updateCookProfileSchema>;
export type ToggleAvailabilityInput = z.infer<typeof toggleAvailabilitySchema>;
export type CookSearchQueryInput = z.infer<typeof cookSearchQuerySchema>;
