import { BaseRepository } from './base.repository.js';
import { Cook, type ICookDocument } from '../models/cook.model.js';
import type { FilterQuery, Types } from 'mongoose';
import type { CookSearchQueryInput } from '../validators/cook.validator.js';
import { mergeNotDeleted } from '../models/common/softDelete.js';
import { VerificationStatus } from '../types/domain.enums.js';

export interface PaginatedCooksResult {
  cooks: ICookDocument[];
  total: number;
  page: number;
  limit: number;
  totalPages: number;
}

export class CookRepository extends BaseRepository<ICookDocument> {
  constructor() {
    super(Cook);
  }

  async findByUserId(userId: string | Types.ObjectId): Promise<ICookDocument | null> {
    return this.model
      .findOne(mergeNotDeleted({ userId }))
      .populate('userId', 'name email phone avatar address isActive')
      .exec();
  }

  async updateByUserId(
    userId: string | Types.ObjectId,
    update: Partial<ICookDocument>,
  ): Promise<ICookDocument | null> {
    return this.model.findOneAndUpdate(
      mergeNotDeleted({ userId }),
      { $set: update },
      { new: true, runValidators: true },
    ).populate('userId', 'name email phone avatar address isActive');
  }

  async findCookById(id: string | Types.ObjectId): Promise<ICookDocument | null> {
    return this.model
      .findOne(mergeNotDeleted({ _id: id }))
      .populate('userId', 'name email phone avatar address isActive')
      .exec();
  }

  async searchCooks(queryParams: CookSearchQueryInput): Promise<PaginatedCooksResult> {
    const {
      search,
      city,
      minExperience,
      maxExperience,
      minPrice,
      maxPrice,
      isAvailable,
      serviceType,
      minRating,
      page,
      limit,
      sort,
      order,
    } = queryParams;

    const filter: FilterQuery<ICookDocument> = mergeNotDeleted({
      verificationStatus: VerificationStatus.VERIFIED,
    });

    if (isAvailable !== undefined) {
      filter.isAvailable = isAvailable;
    }

    if (serviceType) {
      filter.serviceTypes = serviceType;
    }

    if (minRating !== undefined) {
      filter.averageRating = { $gte: minRating };
    }

    if (minExperience !== undefined || maxExperience !== undefined) {
      filter.experienceYears = {};
      if (minExperience !== undefined) filter.experienceYears.$gte = minExperience;
      if (maxExperience !== undefined) filter.experienceYears.$lte = maxExperience;
    }

    if (minPrice !== undefined || maxPrice !== undefined) {
      filter.hourlyRate = {};
      if (minPrice !== undefined) filter.hourlyRate.$gte = minPrice;
      if (maxPrice !== undefined) filter.hourlyRate.$lte = maxPrice;
    }

    if (search) {
      const searchRegex = new RegExp(search, 'i');
      filter.$or = [
        { bio: searchRegex },
        { skills: searchRegex },
        { languages: searchRegex },
      ];
    }

    const sortOption: Record<string, 1 | -1> = {
      [sort]: order === 'asc' ? 1 : -1,
    };

    const skip = (page - 1) * limit;

    let query = this.model
      .find(filter)
      .populate({
        path: 'userId',
        select: 'name email phone avatar address isActive',
        match: city ? { 'address.city': new RegExp(city, 'i'), isActive: true } : { isActive: true },
      })
      .sort(sortOption);

    const allMatching = await query.exec();
    
    // Filter out items where populated userId didn't match (e.g. city filter or inactive user)
    const validCooks = allMatching.filter((doc) => doc.userId !== null && doc.userId !== undefined);
    const total = validCooks.length;
    const paginatedCooks = validCooks.slice(skip, skip + limit);
    const totalPages = Math.ceil(total / limit) || 1;

    return {
      cooks: paginatedCooks,
      total,
      page,
      limit,
      totalPages,
    };
  }
}

export const cookRepository = new CookRepository();
