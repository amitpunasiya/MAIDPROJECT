...
export const DashboardHome: React.FC = () => {
  ...
  const revenueMonthly = stats.revenue?.monthly ?? 0;
  const bookingsTotal = stats.bookings?.total ?? 0;
  const customersTotal = stats.users?.customers ?? stats.users?.total ?? 0;
  const providersTotal = stats.users?.providers ?? 0;
  const activeProviders = stats.users?.activeProviders ?? 0;
        ...
        <Grid item xs={12} sm={6} md={3}>
          <Card elevation={0} sx={{ border: '1px solid #e2e8f0', borderRadius: 3 }}>
            <CardContent>
              ...
              <Typography variant="caption" color="success.main" fontWeight={600} sx={{ display: 'flex', alignItems: 'center', mt: 1 }}>
                <CheckIcon fontSize="small" sx={{ mr: 0.5 }} /> {stats.bookings?.completed ?? 0} Completed
              </Typography>
            </CardContent>
          </Card>
        </Grid>
          ...
          <Paper elevation={0} sx={{ p: 3, border: '1px solid #e2e8f0', borderRadius: 3 }}>
            ...
            <Box sx={{ mb: 3 }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                ...
                <Typography variant="body2" fontWeight={700}>{stats.users?.cooks ?? 0} registered</Typography>
              </Box>
              <LinearProgress
                variant="determinate"
                value={providersTotal ? ((stats.users?.cooks || 0) / providersTotal) * 100 : 0}
                color="primary"
            ...
            <Box sx={{ mb: 3 }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                ...
                <Typography variant="body2" fontWeight={700}>{stats.users?.maids ?? 0} registered</Typography>
              </Box>
              <LinearProgress
                variant="determinate"
                value={providersTotal ? ((stats.users?.maids || 0) / providersTotal) * 100 : 0}
                color="success"
                sx={{ height: 8, borderRadius: 4 }}
              />
            </Box>
            ...
            <Box>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                <Typography variant="body2" fontWeight={600}>Pending KYC Approval</Typography>
                <Typography variant="body2" fontWeight={700}>{stats.users?.pendingProviders ?? 0} pending</Typography>
              </Box>
              <LinearProgress
                variant="determinate"
                value={providersTotal ? ((stats.users?.pendingProviders || 0) / providersTotal) * 100 : 0}
                color="warning"
                sx={{ height: 8, borderRadius: 4 }}
              />
            </Box>
          ...
          <Paper elevation={0} sx={{ p: 3, border: '1px solid #e2e8f0', borderRadius: 3, height: '100%' }}>
            ...
            <List disablePadding>
              <ListItem sx={{ px: 0, py: 1 }}>
                <ListItemText primary="API Server Status" secondary="Connected • Express API v1" />
                <Chip label="OPERATIONAL" color="success" size="small" />
              </ListItem>
              <ListItem sx={{ px: 0, py: 1 }}>
                <ListItemText primary="MongoDB Database" secondary="Connected • Atlas Cluster" />
                <Chip label="HEALTHY" color="success" size="small" />
              </ListItem>
              <ListItem sx={{ px: 0, py: 1 }}>
                <ListItemText primary="JWT Auth & Session Guard" secondary="Active • Token Verification" />
                <Chip label="SECURE" color="info" size="small" />
              </ListItem>
              <ListItem sx={{ px: 0, py: 1 }}>
                <ListItemText primary="Payment & Notification Modules" secondary="Live Webhooks Active" />
